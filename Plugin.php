...
class Plugin extends AbstractPlugin
{
    ...
    {
        // Register the enhanced telegram notifications feature for all registered site types
        // Get already registered site types
        $siteTypes = \App\Facades\SiteType::getAll();

        if (empty($siteTypes)) {
            // If no site types are registered yet, use default ones
            $siteTypes = ['laravel', 'php', 'wordpress', 'phpmyadmin', 'nodejs', 'python', 'static'];
        } else {
            // Extract the site type keys
            $siteTypes = array_keys($siteTypes);
        }
        ...
        foreach ($siteTypes as $siteType) {
            try {
                ...
                // Site type might not be registered yet, skip it
                continue;
            }
        }
